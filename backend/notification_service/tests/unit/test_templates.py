...
def test_transaction_transfer_render():
	_s, b, _ = TRANSACTION_TRANSFER.render(
		{
			"amount": "1000",
			"currency": "RUB",
			"from_account": "ACC1",
...
def test_transaction_incoming_render():
	_s, b, _ = TRANSACTION_INCOMING.render(
		{
			"account_number": "ACC2",
			"amount": "1000",
			"currency": "RUB",
...
def test_account_opened_render():
	_s, b, _ = ACCOUNT_OPENED.render(
		{
			"account_type": "дебетовый",
			"currency": "RUB",
			"account_number": "40817",
		}
	)
...
def test_unlock_code_render():
	_s, b, _ = UNLOCK_CODE.render({"code": "654321"})
	assert "654321" in b
...
def test_no_variable_templates():
	...
	for tmpl in (
		...
		try:
			subj, _body, _ = tmpl.render({"account_number": "x", "code": "1"})
